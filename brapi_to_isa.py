...
def paging(url: object, params: object, data: object, method: object) -> object:
    ...
    while maxcount is None or page < maxcount:
        ...
        if method == 'GET':
            ...
            headers = {}
            r = requests.post(url, params=json.dumps(params).encode('utf-8'), json=data, headers=headers)  # , verify=False
            print(r)
...
def create_isa_study(brapi_study_id):
    ...
    oa_tt = OntologyAnnotation(term="gene sequencing", term_accession="", term_source=oref_tt)
    oref_mt = OntologySource(name="OBI", description="Ontology for Biomedical Investigation")
    oa_mt = OntologyAnnotation(term="nucleotide sequencing", term_accession="", term_source=oref_mt)
...
def create_isa_obs_data_from_obsvars(all_obs_units):
    ...
    for index in range(len(all_obs_units)):
        ...
        for e in range(len(all_obs_units[index]['observations'])):
            data_record = str(all_obs_units[index]['observations'][e]['observationDbId']) + "\t" +\
                          str(all_obs_units[index]['observations'][e]['observationVariableDbId']) + "\t" +\
                          str(all_obs_units[index]['observations'][e]['value']) + "\t" +\
                          str(all_obs_units[index]['observations'][e]['observationTimeStamp']) + "\t" +\
                          str(all_obs_units[index]['observations'][e]['collector'])
...
def main(SERVER):
    ...
    for trial in load_trials():
        ...
        for study in trial['studies']:
            ...
            # GNPIS endpoint exemplar studies ['BTH_Dijon_2000_SetA1' : #'BTH_Chaux_des_Prés_2007_SetA1' 'BTH_Rennes_2003_TECH']
            ...
            if study['studyDbId'] == '1001':
                ...
                # Getting the phenotyping assay table generated by the 'create_isa_study' method by default
                this_assay = study.assays[0]

                # Getting the list of all germplasms used in the study:
                germplasms = get_germplasm_in_study(study_id)

                germ_counter = 0
                # for each germplasm considered as biosource, we retrieve their attributes and create isa characteristics
                ...
                for i in range(len(obsunits)):
                    # initializing set of characteristics and factors for this entry
                    characteristics = []
                    factors = []
                    ...
                    # print("sample: ", this_sample.name)

                    sample_collection_process = Process(executes_protocol=sample_collection_protocol)
                    sample_collection_process.performer = ""
                    sample_collection_process.date = ""
                    ...
                    for j in range(len((obsunits[i]['observations']))):
                        ...
                        phenotyping_process.name = "assay-name_(" + obsunits[i]["observationUnitName"] + ")_" + str(len(obsunits[i]['observations'][j]))
                        ...
                        if 'season' in obsunits[i]['observations'][j].keys():
                            pv = ParameterValue(
                                ...
                                value=OntologyAnnotation(term=str(obsunits[i]['observations'][j]['season']), term_source="",
                                                         term_accession=""))
                        else:
                            pv = ParameterValue(
                                category=ProtocolParameter(parameter_name=OntologyAnnotation(term="season")),
                ...
                try:
                    ...
                    print(e)
                try:
                    write_records_to_file(this_study_id=str(study_id), this_directory=directory, records=variable_records, filetype="t_")
                except IOError as e:
                    print(e)

                # Getting Variable Data to produce Measurement Data File
